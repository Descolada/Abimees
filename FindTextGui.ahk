...
ft_Gui(cmd)
{
  static
  static init:=0
  static ft_FuncBind1:=Func("ft_Gui").Bind("Show")
  static ft_FuncBind2:=Func("ft_Gui").Bind("ScreenShot")
  ...
  if (cmd="MakeMainWindow")
  {
    ...
    Gui, Add, Checkbox, x+30 ys Checked vAddFunc, Add FindText()
    GuiControlGet, p, Pos, AddFunc
    w:=720+15-(px+pw)-15
    Gui, Add, Button, x+15 ys-8 w%w% vTestClip gft_Run, TestClipboard
    ...
    w:=720+15-(px+pw)-5
    Gui, Add, Button, x+5 yp-3 w%w% vApply gft_Run, Apply
...
ft_ShowToolTip(cmd:="")
{
  ...
  static ft_FuncBind4:=Func("ft_ShowToolTip").Bind("ToolTip")
  static ft_FuncBind5:=Func("ft_ShowToolTip").Bind("ToolTipOff")
  ListLines, Off
  if (!ToolTip_Text)
...
ft_Mini_Show(px, py, ww, hh, d:=2)
{
  local
  Gui, ft_Mini_4: +LastFoundExist
  IfWinNotExist
  {
    Loop, 4
    {
      i:=A_Index
      Gui, ft_Mini_%i%: +AlwaysOnTop +ToolWindow -Caption -DPIScale +E0x08000000
      Gui, ft_Mini_%i%: Color, Red
      Gui, ft_Mini_%i%: Show, Hide, Mini
    }
  }
  ...
  Loop, 4
  {
    ...
    Gui, ft_Mini_%i%: Show, NA x%x% y%y% w%w% h%h%
  }
}
...
{
  local
  Gui, ft_Mini_4: +Hwndid
  Loop, 4
    Gui, ft_Mini_%A_Index%: Destroy
